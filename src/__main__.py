...
@click.option(
    "--output",
    help="Output directory path for crispin init & run. Equivalient to nextflow launchDir. Defaults to your current working directory.",
...
def run(main_path, output, _mode, force_all, **kwargs):
    ...
    if not output_dir.is_dir() or not (output_dir / "nextflow.config").exists():
        raise FileNotFoundError(
            f"output directory not initialized: {output_dir}. Hint: you must initialize the output directory with `crispin init --output {output_dir}`"
        )
...
@click.option(
    "--output",
    help="Output directory path for crispin init & run. Equivalent to nextflow launchDir. Defaults to your current working directory.",
    type=click.Path(file_okay=False, dir_okay=True, writable=True),
    default=pathlib.Path.cwd(),
    show_default=False,
)
def init(output):
    ...
    output_dir = output if isinstance(output, pathlib.Path) else pathlib.Path(output)
    ccbr_tools.pkg_util.msg_box(f"Initializing CRISPIN in {output_dir}")
